...
| ALGO-CO-006b | Weighted edge betweenness community detection — Brandes-Dijkstra per-removal weighted edge-betweenness + reverse-dendrogram replay with weighted-modularity best-Q tracking (`edge_betweenness_community_weighted`; reuses `EdgeBetweennessResult`); rejects directed/non-finite/negative weights | community/edge_betweenness.c | ~430 | adapt | CO-006 + modularity_weighted (CO-001) | done | (next) | O(\|V\|·\|E\|·(\|E\|+\|V\| log \|V\|)) | C:2 / py:2 / R:2 |
...
**Phase 0 — complete (37/37)**. **Phase 1 underway**: 87/87 done —
...
CO-006 (Edge betweenness community detection Girvan-Newman 2002 — `edge_betweenness_community`; iteratively remove the highest-betweenness edge via a Brandes unweighted edge-betweenness pass over a per-removal *active* edge set, recording removal order in `removed_edges`, the corresponding `edge_betweenness` at removal (halved for undirected to match centrality convention), then walks removals in reverse to build the binary dendrogram — each removal that re-joins two distinct components is a merge with new cluster id `n + i`, modularity is recomputed via standalone `modularity()` at every merge, and the best-Q membership is densified to `0..nb_clusters` and returned; `EdgeBetweennessResult` also exposes `merges`, `bridges`, `modularity` per merge so callers can replay any cut point; edges are masked via private per-vertex `Vec<EdgeId>` incidence lists with `retain`, never via `graph.delete_edges`, so original edge IDs stay stable for the entire run; ties broken by smallest active edge id matching upstream `igraph_i_which_max_active_ratio`; rejects directed graphs (`Unsupported("directed edge_betweenness_community is ALGO-CO-006c; not yet ported")`); karate Q ≈ 0.40 (k=5) with chosen partition matching standalone `modularity()` to 1e-9, ring-of-4-K5-cliques splits ground-truth k=4, two-K4-with-bridge removes the bridge first; bench: path-10 ~13.5µs / two-K4-bridge ~14.0µs / ring-of-cliques(4×5) ~115.5µs / karate ~998µs — karate runs in ~71% of python-igraph 0.11.9's wall time, ring-of-cliques within noise (Rust 115µs vs python 128µs)),
...
Phase 4 community detection continuation (Walktrap / Infomap remain; directed edge_betweenness_community CO-006c as the next EB follow-up).
