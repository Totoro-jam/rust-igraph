...
#[test]
fn get_all_shortest_paths_dijkstra_three_source_conformance() {
    // ALGO-SP-037: All shortest paths (Dijkstra). Conformance compares nrgeo
    // (path count per vertex), which is implementation-independent.
    use rust_igraph::get_all_shortest_paths_dijkstra;
    run_conformance("get_all_shortest_paths_dijkstra", |g, params| {
        #[allow(clippy::cast_possible_truncation)]
        ...
        let weights: Vec<f64> = params["weights"]
            ...
            .collect();
        let result = get_all_shortest_paths_dijkstra(g, source, &weights)
            .expect("get_all_shortest_paths_dijkstra");
        let nrgeo: Vec<serde_json::Value> =
            result.nrgeo.iter().map(|&n| serde_json::json!(n)).collect();
        serde_json::json!(nrgeo)
    });
}
...
#[test]
#[allow(clippy::too_many_lines)] // three-source dispatch + directed/canon compare + array dims
fn triangular_lattice_three_source_conformance() {
    // Planar triangular lattice (`lattices.c:igraph_triangular_lattice`).
    // The `dims` slice doubles as a shape selector: 1 element → triangle,
    // 2 → quasi-rectangle, 3 → hexagon. `directed && mutual` doubles
    // every edge in opposite orientations.
    ...
    fn parse_u32_array(case: &Conformance, key: &str, path: &std::path::Path) -> Vec<u32> {
        let Some(arr) = case.params.get(key).and_then(serde_json::Value::as_array) else {
            panic!(
                "triangular_lattice fixture {}: param `{}` missing or not array",
                path.display(),
                key
            );
        };
        arr.iter()
            .map(|v| {
                u32::try_from(v.as_u64().unwrap_or_else(|| {
                    panic!(
                        "triangular_lattice fixture {}: param `{}` element not u64",
                        path.display(),
                        key
                    )
                }))
    ...
    for src in ["c", "py", "r"] {
        let dir = workspace_root()
            .join("tests/conformance")
            .join(src)
            .join("triangular_lattice");
        if !dir.is_dir() {
            continue;
        }
        for entry in fs::read_dir(&dir).expect("read triangular_lattice fixture dir") {
            let entry = entry.expect("dir entry");
            let path = entry.path();
            if path.extension().and_then(|s| s.to_str()) != Some("json") {
                continue;
            ...
            let bytes = fs::read(&path).expect("read fixture file");
            let case: Conformance = serde_json::from_slice(&bytes)
                .expect("parse triangular_lattice conformance fixture JSON");
            assert_eq!(case.algo, "triangular_lattice");

            let dims = parse_u32_array(&case, "dims", &path);
            let directed = er_param_bool(&case, "directed", &path);
            let mutual = er_param_bool(&case, "mutual", &path);

            let graph = triangular_lattice(&dims, directed, mutual)
                .expect("triangular_lattice should succeed on conformance fixtures");
            ...
            let want_edges_raw = case
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "triangular_lattice fixture {}: expected.edges missing or not array",
                        path.display()
                    )
                });
            let mut want_pairs: Vec<(u32, u32)> = Vec::with_capacity(want_edges_raw.len());
            for v in want_edges_raw {
                let pair = v.as_array().unwrap_or_else(|| {
                    panic!(
                        "triangular_lattice fixture {}: expected.edges entry not array",
                        path.display()
                    )
                });
                let u = u32::try_from(pair[0].as_u64().expect("edge endpoint u64"))
            ...
            let got_pairs: Vec<(u32, u32)> = (0..n_edges)
                .map(|eid| {
                    graph
                        .edge(eid)
                        .expect("conformance triangular_lattice edge id in bounds")
                })
                .collect();

            // For directed graphs we compare (u, v) verbatim — the
            // mutual flag distinguishes (a,b) from (b,a). For undirected
            // we canonicalise to (min, max) because the per-vertex
            // emission order is not portable across the three sources.
            let mut got_canon: Vec<(u32, u32)> = got_pairs
                .iter()
                .map(|&(u, v)| if want_directed { (u, v) } else { canon(u, v) })
                .collect();
            let mut want_canon: Vec<(u32, u32)> = want_pairs
                .iter()
                .map(|&(u, v)| if want_directed { (u, v) } else { canon(u, v) })
                .collect();
            got_canon.sort_unstable();
            ...
            assert_eq!(
                got_canon,
                want_canon,
                "triangular_lattice edge multiset mismatch in {}\n  source: {}\n  origin: {}",
                path.display(),
                case.source,
                case.origin,
            );
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen_sources.contains(src),
            "no triangular_lattice fixtures from source {src}"
        );
    }
}

#[test]
#[allow(clippy::too_many_lines)] // two-source dispatch + directed/canon compare + array dims
fn hexagonal_lattice_two_source_conformance() {
    // Planar hexagonal (honeycomb) lattice
    // (`lattices.c:igraph_hexagonal_lattice`). Same shape selector as
    // `triangular_lattice` (1→triangle, 2→quasi-rectangle, 3→hexagon)
    // but every vertex has degree at most 3 and only right + (odd-k)
    // up-left neighbours are emitted. rigraph does not bind this
    // constructor, so the R lane is intentionally absent.
    ...
    fn parse_u32_array(case: &Conformance, key: &str, path: &std::path::Path) -> Vec<u32> {
        let Some(arr) = case.params.get(key).and_then(serde_json::Value::as_array) else {
            panic!(
                "hexagonal_lattice fixture {}: param `{}` missing or not array",
                path.display(),
                key
            );
        };
        arr.iter()
            .map(|v| {
                u32::try_from(v.as_u64().unwrap_or_else(|| {
                    panic!(
                        "hexagonal_lattice fixture {}: param `{}` element not u64",
                        path.display(),
                        key
                    )
                }))
    ...
    for src in ["c", "py"] {
        let dir = workspace_root()
            .join("tests/conformance")
            .join(src)
            .join("hexagonal_lattice");
        if !dir.is_dir() {
            continue;
        }
        for entry in fs::read_dir(&dir).expect("read hexagonal_lattice fixture dir") {
            let entry = entry.expect("dir entry");
            let path = entry.path();
            if path.extension().and_then(|s| s.to_str()) != Some("json") {
                continue;
            ...
            let bytes = fs::read(&path).expect("read fixture file");
            let case: Conformance = serde_json::from_slice(&bytes)
                .expect("parse hexagonal_lattice conformance fixture JSON");
            assert_eq!(case.algo, "hexagonal_lattice");

            let dims = parse_u32_array(&case, "dims", &path);
            let directed = er_param_bool(&case, "directed", &path);
            let mutual = er_param_bool(&case, "mutual", &path);

            let graph = hexagonal_lattice(&dims, directed, mutual)
                .expect("hexagonal_lattice should succeed on conformance fixtures");
            ...
            let want_edges_raw = case
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "hexagonal_lattice fixture {}: expected.edges missing or not array",
                        path.display()
                    )
                });
            let mut want_pairs: Vec<(u32, u32)> = Vec::with_capacity(want_edges_raw.len());
            for v in want_edges_raw {
                let pair = v.as_array().unwrap_or_else(|| {
                    panic!(
                        "hexagonal_lattice fixture {}: expected.edges entry not array",
                        path.display()
                    )
                });
                let u = u32::try_from(pair[0].as_u64().expect("edge endpoint u64"))
            ...
            let got_pairs: Vec<(u32, u32)> = (0..n_edges)
                .map(|eid| {
                    graph
                        .edge(eid)
                        .expect("conformance hexagonal_lattice edge id in bounds")
                })
                .collect();
            ...
            assert_eq!(
                got_canon,
                want_canon,
                "hexagonal_lattice edge multiset mismatch in {}\n  source: {}\n  origin: {}",
                path.display(),
                case.source,
                case.origin,
            );
    ...
    for src in ["c", "py"] {
        assert!(
            seen_sources.contains(src),
            "no hexagonal_lattice fixtures from source {src}"
        );
    }
}
...
fn check_famous_fixture(case: &Conformance, path: &std::path::Path) {
    ...
    let name = case
        ...
        .get("name")
        .and_then(serde_json::Value::as_str)
        .unwrap_or_else(|| panic!("famous fixture {}: params.name missing", path.display()));

    let g = famous(name).expect("famous() should succeed on conformance fixtures");
    ...
    assert_eq!(
        g.vcount(),
        want_vertices,
        "famous vcount mismatch in {}",
        path.display()
    );
    assert_eq!(
        g.is_directed(),
        want_directed,
        "famous directed mismatch in {}",
        path.display()
    );
    assert_eq!(
        g.ecount() as u64,
        want_edges_count,
        "famous ecount mismatch in {}",
        path.display()
    );
    ...
    let edges_field = case
        .expected
        .get("edges")
        .unwrap_or_else(|| panic!("famous fixture {}: expected.edges missing", path.display()));
    if edges_field.is_null() {
        return;
    }
    let want_edges_raw = edges_field.as_array().unwrap_or_else(|| {
        panic!(
            "famous fixture {}: expected.edges not array/null",
            path.display()
        )
    });
    ...
    for v in want_edges_raw {
        let pair = v
            .as_array()
            .unwrap_or_else(|| panic!("famous fixture {}: edge not array", path.display()));
        let u = u32::try_from(pair[0].as_u64().expect("u64")).expect("u32");
        let w = u32::try_from(pair[1].as_u64().expect("u64")).expect("u32");
        let key = if u <= w { (u, w) } else { (w, u) };
        *want_ms.entry(key).or_insert(0) += 1;
    ...
    assert_eq!(
        got_ms,
        want_ms,
        "famous edge multiset mismatch in {}\n  source: {}\n  origin: {}",
        path.display(),
        case.source,
        case.origin,
    );
...
#[test]
fn famous_three_source_conformance() {
    // `igraph_famous(name)` is exercised by all three official bindings:
    // C directly, python-igraph's `Graph.Famous(name)`, and rigraph's
    // `make_graph("<name>")` (which dispatches to `famous_impl`). The
    // edge tables are static so the canonical-pair multiset check is the
    // strongest equivalent of "bit-for-bit identical to igraph C".
    let mut seen_sources = std::collections::BTreeSet::<&'static str>::new();
    for src in ["c", "py", "r"] {
        let dir = workspace_root()
            .join("tests/conformance")
        ...
        for entry in fs::read_dir(&dir).expect("read famous fixture dir") {
            let entry = entry.expect("dir entry");
            let path = entry.path();
            if path.extension().and_then(|s| s.to_str()) != Some("json") {
                continue;
            ...
            let bytes = fs::read(&path).expect("read fixture file");
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse famous conformance fixture JSON");
            assert_eq!(case.algo, "famous");
            assert_eq!(case.source, src);
            check_famous_fixture(&case, &path);
            seen_sources.insert(src);
        }
    }
    for src in ["c", "py", "r"] {
        assert!(
            seen_sources.contains(src),
            "no famous fixtures from source {src}"
        );
    }
}

fn check_atlas_fixture(case: &Conformance, path: &std::path::Path) {
    ...
    let number = u32::try_from(
        case.params
            .get("number")
            .and_then(serde_json::Value::as_u64)
            .unwrap_or_else(|| panic!("atlas fixture {}: params.number missing", path.display())),
    )
    .expect("atlas number fits u32");

    let g = atlas(number).expect("atlas() should succeed on conformance fixtures");
    ...
    assert_eq!(
        g.vcount(),
        want_vertices,
        "atlas vcount mismatch in {}",
        path.display()
    );
    assert_eq!(
        g.is_directed(),
        want_directed,
        "atlas directed mismatch in {}",
        path.display()
    );
    assert_eq!(
        g.ecount() as u64,
        want_edges_count,
        "atlas ecount mismatch in {}",
        path.display()
    );
    ...
    let edges_field = case
        .expected
        .get("edges")
        .unwrap_or_else(|| panic!("atlas fixture {}: expected.edges missing", path.display()));
    if edges_field.is_null() {
        return;
    }
    let want_edges_raw = edges_field.as_array().unwrap_or_else(|| {
        panic!(
            "atlas fixture {}: expected.edges not array/null",
            path.display()
        )
    });
    ...
    for v in want_edges_raw {
        let pair = v
            .as_array()
            .unwrap_or_else(|| panic!("atlas fixture {}: edge not array", path.display()));
        let u = u32::try_from(pair[0].as_u64().expect("u64")).expect("u32");
        let w = u32::try_from(pair[1].as_u64().expect("u64")).expect("u32");
        let key = if u <= w { (u, w) } else { (w, u) };
        *want_ms.entry(key).or_insert(0) += 1;
    ...
    assert_eq!(
        got_ms,
        want_ms,
        "atlas edge multiset mismatch in {}\n  source: {}\n  origin: {}",
        path.display(),
        case.source,
        case.origin,
    );
...
#[test]
fn atlas_three_source_conformance() {
    // `igraph_atlas(number)` is the same C entry point reached from
    // python-igraph's `Graph.Atlas(n)` and rigraph's `graph_from_atlas(n)`.
    // We compare a representative slice of indices spanning every cell
    // boundary (null graphs, K_n for n=2..7, and the two graphs that
    // python-igraph's own test_atlas.py drops at line 174).
    let mut seen_sources = std::collections::BTreeSet::<&'static str>::new();
    for src in ["c", "py", "r"] {
        let dir = workspace_root()
            .join("tests/conformance")
        ...
        for entry in fs::read_dir(&dir).expect("read atlas fixture dir") {
            let entry = entry.expect("dir entry");
            let path = entry.path();
            if path.extension().and_then(|s| s.to_str()) != Some("json") {
                continue;
            ...
            let bytes = fs::read(&path).expect("read fixture file");
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse atlas conformance fixture JSON");
            assert_eq!(case.algo, "atlas");
            assert_eq!(case.source, src);
            check_atlas_fixture(&case, &path);
            seen_sources.insert(src);
        }
    }
    for src in ["c", "py", "r"] {
        assert!(
            seen_sources.contains(src),
            "no atlas fixtures from source {src}"
        );
    }
}
...
#[test]
#[allow(clippy::too_many_lines)] // three-source dispatch + invariant checks
fn citing_cited_type_game_three_source_conformance() {
    // Citing-cited-type citation game (ALGO-GN-029). RNG state is not
    // portable; each fixture pins parameters and asserts structural
    // invariants:
    //   * vcount = nodes (exact);
    //   * directed flag exact;
    //   * ecount lies in a band [ecount_min, ecount_max] — for
    //     positive-pref runs both bounds equal (n-1)*eps;
    //   * citing_cited_type NEVER self-loops: even under the uniform
    //     fallback (sums[citing_type[i]] == 0) the RNG samples [0, i),
    //     and the new vertex i is only added to the trees AFTER its
    //     out-edges are drawn. So no_self_loops is structurally
    //     guaranteed and asserted whenever the fixture flags it;
    //   * citing_cited_type MAY produce multi-edges when eps≥2
    //     (multiple draws at one step may select the same target);
    //     simplicity is therefore NOT asserted here.
    ...
    fn parse_u32_array(case: &Conformance, key: &str, path: &std::path::Path) -> Vec<u32> {
        case.params
            ...
            .unwrap_or_else(|| {
                panic!(
                    "citing_cited_type fixture {}: param `{}` missing or not array",
                    path.display(),
                    key
                )
            })
            ...
            .map(|cell| {
                let n = cell.as_u64().unwrap_or_else(|| {
                    panic!(
                        "citing_cited_type fixture {}: param `{}` cell is not u64",
                        path.display(),
                        key
                    )
                });
                u32::try_from(n).unwrap_or_else(|_| {
                    panic!(
                        "citing_cited_type fixture {}: param `{}` cell {} does not fit u32",
                        path.display(),
                        key,
                        n
                    )
    ...
    for src in ["c", "py", "r"] {
        ...
        for entry in fs::read_dir(&dir).expect("read fixture dir") {
            ...
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse conformance fixture JSON");
            assert_eq!(case.algo, "citing_cited_type_game");

            let nodes = er_param_u32(&case, "nodes", &path);
            let types = parse_u32_array(&case, "types", &path);
            let pref_rows = er_param_f64_matrix(&case, "pref", &path);
            ...
            let graph =
                citing_cited_type_game(nodes, &types, &pref_views, edges_per_step, directed, seed)
                    .expect("citing_cited_type_game should succeed on conformance fixtures");
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen_sources.contains(src),
            "no citing_cited_type_game fixtures from source {src}"
        );
    }
}

#[test]
fn max_flow_value_three_source_conformance() {
    ...
    for src in ["c", "py", "r"] {
        ...
        for entry in std::fs::read_dir(&dir).expect("read fixture dir") {
            ...
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse conformance fixture JSON");
            assert_eq!(case.algo, "max_flow_value");
            let g = build_graph(&case.graph);
            let source = u32::try_from(
                case.params
                    .get("source")
            ...
            let value =
                rust_igraph::max_flow_value(&g, source, target, cap).expect("max_flow_value");
            let actual = serde_json::json!(value);
            assert!(
                json_approx_eq(&actual, &case.expected),
                "max_flow_value conformance failure\n  fixture: {}\n  source:  {}\n  origin:  {}\n  actual:   {}\n  expected: {}",
                path.display(),
                case.source,
                case.origin,
                actual,
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen.contains(src),
            "no max_flow_value fixtures from source {src}"
        );
    }
}
...
#[test]
fn st_edge_connectivity_three_source_conformance() {
    // ALGO-FL-011: thin wrapper over `max_flow_value` with unit caps,
    // cast to integer. Fixtures from C, py, R pin the wrapper to the
    // same per-source ground-truth integer values.
    let mut seen = std::collections::HashSet::<&'static str>::new();
    for src in ["c", "py", "r"] {
        let dir = workspace_root()
            .join("tests/conformance")
            .join(src)
            .join("st_edge_connectivity");
        if !dir.is_dir() {
            continue;
        }
        for entry in std::fs::read_dir(&dir).expect("read fixture dir") {
            ...
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse conformance fixture JSON");
            assert_eq!(case.algo, "st_edge_connectivity");
            let g = build_graph(&case.graph);
            let source = u32::try_from(
                case.params
                    .get("source")
            ...
            .expect("target fits in u32");
            let value = rust_igraph::st_edge_connectivity(&g, source, target)
                .expect("st_edge_connectivity");
            let actual = serde_json::json!(value);
            assert!(
                json_approx_eq(&actual, &case.expected),
                "st_edge_connectivity conformance failure\n  fixture: {}\n  source:  {}\n  origin:  {}\n  actual:   {}\n  expected: {}",
                path.display(),
                case.source,
                case.origin,
                actual,
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen.contains(src),
            "no st_edge_connectivity fixtures from source {src}"
        );
    }
}

#[test]
fn edge_disjoint_paths_three_source_conformance() {
    // ALGO-FL-012: thin wrapper over `max_flow_value` with unit caps,
    // cast to integer (Menger's theorem). Fixtures from C, py, R pin
    // the wrapper to the same per-source ground-truth integer values.
    let mut seen = std::collections::HashSet::<&'static str>::new();
    for src in ["c", "py", "r"] {
        let dir = workspace_root()
            .join("tests/conformance")
            .join(src)
            .join("edge_disjoint_paths");
        if !dir.is_dir() {
            continue;
        }
        for entry in std::fs::read_dir(&dir).expect("read fixture dir") {
            ...
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse conformance fixture JSON");
            assert_eq!(case.algo, "edge_disjoint_paths");
            let g = build_graph(&case.graph);
            let source = u32::try_from(
                case.params
                    .get("source")
            ...
            .expect("target fits in u32");
            let value =
                rust_igraph::edge_disjoint_paths(&g, source, target).expect("edge_disjoint_paths");
            let actual = serde_json::json!(value);
            assert!(
                json_approx_eq(&actual, &case.expected),
                "edge_disjoint_paths conformance failure\n  fixture: {}\n  source:  {}\n  origin:  {}\n  actual:   {}\n  expected: {}",
                path.display(),
                case.source,
                case.origin,
                actual,
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen.contains(src),
            "no edge_disjoint_paths fixtures from source {src}"
        );
    }
}
...
#[test]
fn vertex_connectivity_three_source_conformance() {
    // ALGO-FL-015: global cohesion — minimum number of internal vertices
    // whose removal disconnects some pair of vertices. Computed as
    // min_{s != t} st_vertex_connectivity(s, t, NumberOfNodes) with
    // optional cheap short-circuits when `checks=true`.
    use rust_igraph::vertex_connectivity;
    ...
    for src in ["c", "py", "r"] {
        ...
        for entry in std::fs::read_dir(&dir).expect("read fixture dir") {
            ...
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse conformance fixture JSON");
            assert_eq!(case.algo, "vertex_connectivity");
            let g = build_graph(&case.graph);
            let checks = case
                .params
                .get("checks")
                .and_then(serde_json::Value::as_bool)
                .unwrap_or(true);
            let value = vertex_connectivity(&g, checks).expect("vertex_connectivity");
            let actual = serde_json::json!(value);
            assert!(
                json_approx_eq(&actual, &case.expected),
                "vertex_connectivity conformance failure\n  fixture: {}\n  source:  {}\n  origin:  {}\n  actual:   {}\n  expected: {}",
                path.display(),
                case.source,
                case.origin,
                actual,
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen.contains(src),
            "no vertex_connectivity fixtures from source {src}"
        );
    }
}
...
#[test]
#[allow(clippy::too_many_lines)]
fn recent_degree_aging_game_three_source_conformance() {
    // Recent-degree-aging PA (ALGO-GN-032). RNG state is not portable;
    // each fixture pins SplitMix64 output and asserts structural
    // invariants:
    //   * vcount = nodes (exact);
    //   * directed flag exact;
    //   * ecount = (nodes - 1) * m exactly when `outseq` is None;
    //   * NEVER self-loops by construction (search_bounded clamps to
    //     [0, i) before vertex i is inserted).
    ...
    for src in ["c", "py", "r"] {
        ...
        for entry in fs::read_dir(&dir).expect("read fixture dir") {
            ...
            let case: Conformance =
                serde_json::from_slice(&bytes).expect("parse conformance fixture JSON");
            assert_eq!(case.algo, "recent_degree_aging_game");
            ...
            let graph = recent_degree_aging_game(
                nodes,
                m,
                None,
                outpref,
            ...
            .expect("recent_degree_aging_game should succeed on conformance fixtures");
    ...
    for src in ["c", "py", "r"] {
        assert!(
            seen_sources.contains(src),
            "no recent_degree_aging_game fixtures from source {src}"
        );
    }
}
...
/// `relative_neighborhood_graph` (ALGO-GEO-004) is a point-set algorithm (build
/// the RNG of N points in d-space), not a graph→graph transform. python-igraph
/// 0.11.9 and rigraph do NOT expose `igraph_relative_neighborhood_graph`, so the
/// only upstream conformance source is igraph C. The fixture encodes the
/// authentic golden output published in igraph's own `beta_skeletons.out`: a
/// triangular lattice, the canonical RNG witness whose every edge sits on the
/// open-lune boundary (the closed β = 2 lune skeleton on the same points is
/// empty). The result edge set is canonical and compared exactly.
#[test]
fn relative_neighborhood_graph_c_conformance() {
    use rust_igraph::relative_neighborhood_graph;

    let cases = load_all("relative_neighborhood_graph");
    assert!(
        !cases.is_empty(),
        "no relative_neighborhood_graph conformance fixtures found — did you run \
         `.venv/bin/python -m scripts.test_extract.from_c --algo relative_neighborhood_graph`?"
    );

    let mut seen_c = 0usize;
    for (path, case) in cases {
        assert_eq!(case.algo, "relative_neighborhood_graph");
        // C is the sole upstream source for this function.
        assert_eq!(
            case.source,
            "c",
            "unexpected non-C relative_neighborhood_graph fixture {}",
            path.display()
        );
        ...
        let g = relative_neighborhood_graph(&points).expect("relative_neighborhood_graph");
        assert_eq!(
            g.vcount(),
            exp_n,
            "relative_neighborhood_graph vertex-count mismatch\n  fixture: {}\n  origin:  {}",
            path.display(),
            case.origin,
        );
        let mut actual_edges: Vec<(u32, u32)> = (0..g.ecount())
        ...
        assert_eq!(
            actual_edges,
            exp_edges,
            "relative_neighborhood_graph edge-set mismatch\n  fixture: {}\n  origin:  {}",
            path.display(),
            case.origin,
        );
        seen_c += 1;
...
/// `lune_beta_skeleton` (ALGO-GEO-006) is a point-set algorithm (build the
/// lune-based β-skeleton of N points in d-space for a shape parameter β), not a
/// graph→graph transform. python-igraph 0.11.9 and rigraph do NOT expose
/// `igraph_lune_beta_skeleton`, so the only upstream conformance source is
/// igraph C. The fixtures encode the authentic golden output published in
/// igraph's own `beta_skeletons.out`: a 25-point random 2-D set at β = 2, β = 1
/// (the Gabriel reduction) and β = 0.5 (the perpendicular-centre construction),
/// plus a 10-point 3-D set at β = 2. The result edge set is canonical and
/// compared exactly.
#[test]
fn lune_beta_skeleton_c_conformance() {
    use rust_igraph::lune_beta_skeleton;

    let cases = load_all("lune_beta_skeleton");
    assert!(
        !cases.is_empty(),
        "no lune_beta_skeleton conformance fixtures found — did you run \
         `.venv/bin/python -m scripts.test_extract.from_c --algo lune_beta_skeleton`?"
    );

    let mut seen_c = 0usize;
    for (path, case) in cases {
        assert_eq!(case.algo, "lune_beta_skeleton");
        assert_eq!(
            case.source,
            "c",
            "unexpected non-C lune_beta_skeleton fixture {}",
            path.display()
        );
        ...
        let g = lune_beta_skeleton(&points, beta).expect("lune_beta_skeleton");
        assert_eq!(
            g.vcount(),
            exp_n,
            "lune_beta_skeleton vertex-count mismatch\n  fixture: {}\n  origin:  {}",
            path.display(),
            case.origin,
        );
        let mut actual_edges: Vec<(u32, u32)> = (0..g.ecount())
        ...
        assert_eq!(
            actual_edges,
            exp_edges,
            "lune_beta_skeleton edge-set mismatch\n  fixture: {}\n  origin:  {}",
            path.display(),
            case.origin,
        );
        seen_c += 1;
