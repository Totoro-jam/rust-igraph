//! ALGO-GN-016 example: callaway-traits / sample-traits-callaway
//! diagnostics.
//!
//! Grows a 2 000-vertex undirected callaway-traits graph with
//! `types = 3` and a strongly assortative pref matrix (high diagonal,
//! low off-diagonal). The output highlights three signatures of the
//! Callaway et al. (2001) model that distinguish it from
//! `establishment_game`:
//!   * the per-type vertex count tracks `type_dist` exactly (every
//!     vertex draws a type up front, even the seed vertices),
//!   * within-type edges dominate when the pref matrix is
//!     diagonal-heavy,
//!   * self-loops and multi-edges are allowed by construction (both
//!     candidate endpoints are drawn uniformly from the existing
//!     population).
//!
//! Run: `cargo run --example callaway_traits_demo`.
...
use rust_igraph::{Graph, callaway_traits_game};
...
/// Counts self-loops and parallel edges. `callaway_traits` is NOT
/// simple by construction, so reporting these makes the contrast with
/// `establishment_game` concrete.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (g, types) = callaway_traits_game(
        n,
        num_types,
        edges_per_step,
        Some(&type_dist),
    ...
    println!(
        "callaway_traits: n = {n}, types = {num_types}, edges_per_step = {edges_per_step}, type_dist = [0.50, 0.25, 0.25]"
    );
    println!("  edges                    = {m}");
